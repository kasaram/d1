...
# Create a DataFrame for error records with an additional column for error messages
error_records = error_records.map(lambda x: x + (x[1],)).toDF(["sl_no", "cis_code", "enitity_name", "model_name", "model_version", "segment", "definitive_grade", "definitive_pd", "cascade_flag", "country_of_operations", "last_grading_date", "error"])
